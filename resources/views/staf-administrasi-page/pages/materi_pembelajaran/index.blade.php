                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex gap-2 justify-content-end align-items-center">
                            <a href={{ route('dashboard.staf_administrasi.materi_pembelajaran.create') }}>
                            ...
                            <table id="basic_datatables" class="display table table-striped table-hover">
                                ...
                                <tbody>
                                    @php
                                        $index = 1;
                                    @endphp
                                    @foreach ($materi_pembelajaran as $item)
                                        <tr>
                                            <td>{{ $index++ }}</td>
                                            <td>{{ $item->judul }}</td>
                                            <td>{{ $item->topik }}</td>
                                            <td class="d-flex flex-column flex-lg-row align-items-center  gap-2">
                                                <a
                                                    href={{ route('dashboard.staf_administrasi.materi_pembelajaran.show', ['materi_pembelajaran' => $item->id]) }}><button
                                                        type="button"
                                                        class="btn btn-sm btn-primary p-2 py-1 fs-6 rounded-3"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Read data mata pelajaran : {{ $item->name }}"><i
                                                            class="fas fa-clipboard-list" aria-hidden="true"></i>
                                                    </button></a>
                                                <a
                                                    href={{ route('dashboard.staf_administrasi.materi_pembelajaran.edit', ['materi_pembelajaran' => $item->id]) }}><button
                                                        class="btn btn-sm btn-success p-2 py-1 fs-6 rounded-3"
                                                        title="Edit data mata pelajaran : {{ $item->name }}"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Edit Data"><i class="fas fa-edit"
                                                            aria-hidden="true"></i></button></a>

                                                <button type="button" class="btn btn-sm btn-danger p-2 py-1 fs-6 rounded-3"
                                                    onclick="WarningAlert('delete', '/dashboard/staf_administrasi/materi_pembelajaran/{{ $item->id }}', 'Delete User?', `Apakah anda yakin ingin menghapus user '{{ $item->name }}' ?`)"
                                                    title="Remove data mata pelajaran : {{ $item->name }}" data-bs-toggle="tooltip"
