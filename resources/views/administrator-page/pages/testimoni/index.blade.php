                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex gap-2 justify-content-end align-items-center">
                            <a href={{ route('dashboard.administrator.testimoni.create') }}>
                                    ...
                                    @foreach ($testimonies as $testimoni)
                                        <tr>
                                            ...
                                            <td class="d-flex flex-column flex-lg-row align-items-center  gap-2">
                                                <a
                                                    href={{ route('dashboard.administrator.testimoni.show', ['testimoni' => $testimoni->id]) }}><button
                                                        type="button"
                                                        class="btn btn-sm btn-primary p-2 py-1 fs-6 rounded-3"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Read data testimoni : {{ $testimoni->title }}"><i
                                                            class="fas fa-clipboard-list" aria-hidden="true"></i>
                                                    </button></a>
                                                <a
                                                    href={{ route('dashboard.administrator.testimoni.edit', ['testimoni' => $testimoni->id]) }}><button
                                                        class="btn btn-sm btn-success p-2 py-1 fs-6 rounded-3"
                                                        title="Edit data testimoni : {{ $testimoni->title }}"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Edit Data"><i class="fas fa-edit"
                                                            aria-hidden="true"></i></button></a>

                                                <button type="button" class="btn btn-sm btn-danger p-2 py-1 fs-6 rounded-3"
                                                    onclick="WarningAlert('delete', '/dashboard/administrator/testimoni/{{ $testimoni->id }}', 'Delete User?', `Apakah anda yakin ingin menghapus user '{{ $testimoni->title }}' ?`)"
                                                    title="Remove data testimoni : {{ $testimoni->title }}" data-bs-toggle="tooltip"
