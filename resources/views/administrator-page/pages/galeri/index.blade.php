                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex gap-2 justify-content-end align-items-center">
                            <a href={{ route('dashboard.administrator.galeri.create') }}>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($galeri as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="image-preview mt-3 w-100" style="border-radius: 5px;">
                                            ...
                                            <td class="d-flex flex-column flex-lg-row align-items-center  gap-2">
                                                <a
                                                    href={{ route('dashboard.administrator.galeri.show', ['ulid' => $item->ulid]) }}><button
                                                        type="button"
                                                        class="btn btn-sm btn-primary p-2 py-1 fs-6 rounded-3"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Read data galeri : {{ $item->title }}"><i
                                                            class="fas fa-clipboard-list" aria-hidden="true"></i>
                                                    </button></a>
                                                <a
                                                    href={{ route('dashboard.administrator.galeri.edit', ['ulid' => $item->ulid]) }}><button
                                                        class="btn btn-sm btn-success p-2 py-1 fs-6 rounded-3"
                                                        title="Edit data galeri : {{ $item->title }}"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Edit Data"><i class="fas fa-edit"
                                                            aria-hidden="true"></i></button></a>

                                                <button type="button" class="btn btn-sm btn-danger p-2 py-1 fs-6 rounded-3"
                                                    onclick="WarningAlert('delete', '/dashboard/administrator/galeri/{{ $item->ulid }}', 'Delete User?', `Apakah anda yakin ingin menghapus user '{{ $item->title }}' ?`)"
                                                    title="Remove data galeri : {{ $item->title }}"
