                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex gap-2 justify-content-end align-items-center">
                            <a href={{ route('dashboard.administrator.kategori-galeri.create') }}>
                                    ...
                                    @foreach ($kategori as $item)
                                        <tr>
                                            ...
                                            <td class="d-flex flex-column flex-lg-row align-items-center  gap-2">
                                                <a
                                                    href={{ route('dashboard.administrator.kategori-galeri.show', ['kategori_galeri' => $item->id]) }}><button
                                                        type="button"
                                                        class="btn btn-sm btn-primary p-2 py-1 fs-6 rounded-3"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Read data kategori-galeri : {{ $item->title }}"><i
                                                            class="fas fa-clipboard-list" aria-hidden="true"></i>
                                                    </button></a>
                                                <a
                                                    href={{ route('dashboard.administrator.kategori-galeri.edit', ['kategori_galeri' => $item->id]) }}><button
                                                        class="btn btn-sm btn-success p-2 py-1 fs-6 rounded-3"
                                                        title="Edit data kategori-galeri : {{ $item->title }}"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Edit Data"><i class="fas fa-edit"
                                                            aria-hidden="true"></i></button></a>

                                                <button type="button" class="btn btn-sm btn-danger p-2 py-1 fs-6 rounded-3"
                                                    onclick="WarningAlert('delete', '/dashboard/administrator/kategori-galeri/{{ $item->id }}', 'Delete User?', `Apakah anda yakin ingin menghapus user '{{ $item->title }}' ?`)"
                                                    title="Remove data kategori-galeri : {{ $item->title }}"
