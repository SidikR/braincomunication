                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex gap-2 justify-content-end align-items-center">
                            <a href={{ route('dashboard.administrator.fasilitas.create') }}>
                                    ...
                                    @foreach ($fasilitases as $fasilitas)
                                        <tr>
                                            ...
                                            <td class="d-flex flex-column flex-lg-row align-items-center h-100 gap-2">
                                                <a
                                                    href={{ route('dashboard.administrator.fasilitas.show', ['fasilita' => $fasilitas->id]) }}><button
                                                        type="button"
                                                        class="btn btn-sm btn-primary p-2 py-1 fs-6 rounded-3"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Read data fasilitas : {{ $fasilitas->title }}"><i
                                                            class="fas fa-clipboard-list" aria-hidden="true"></i>
                                                    </button></a>
                                                <a
                                                    href={{ route('dashboard.administrator.fasilitas.edit', ['fasilita' => $fasilitas->id]) }}><button
                                                        class="btn btn-sm btn-success p-2 py-1 fs-6 rounded-3"
                                                        title="Edit data fasilitas : {{ $fasilitas->title }}"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Edit Data"><i class="fas fa-edit"
                                                            aria-hidden="true"></i></button></a>

                                                <button type="button" class="btn btn-sm btn-danger p-2 py-1 fs-6 rounded-3"
                                                    onclick="WarningAlert('delete', '/dashboard/administrator/fasilitas/{{ $fasilitas->id }}', 'Delete User?', `Apakah anda yakin ingin menghapus user '{{ $fasilitas->title }}' ?`)"
                                                    title="Remove data fasilitas : {{ $fasilitas->title }}"
